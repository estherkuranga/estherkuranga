      ...
      >
        ...
        <div className="max-sm:hidden w-full sm:w-4/6 m-auto mt-8 animate-fade-in-up animation-delay-100">
          <ul className="w-full flex items-center justify-center space-x-4 sm:space-x-6 md:space-x-8 mt-4">
            {categories.map((category, index) => (
              <li
                key={index}
                className="text-lg sm:text-xl text-black dark:text-[#FBFBFB] cursor-pointer hover:text-[#FF6300] focus:outline-none focus:ring-2 focus:ring-[#FF6300] focus:ring-offset-2 rounded transition-all duration-300 hover:scale-105 animate-fade-in-up"
        ...
        <div className="w-full sm:mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 justify-center m-auto animate-fade-in-up animation-delay-300">
          {projectData.map((item, index) => (
            <div
              key={index}
              className="bg-white dark:bg-[#2C2C2C] p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-500 ease-out hover:scale-105 hover:-translate-y-2 animate-fade-in-up"
            ...
            >
              ...
              <Button
                variant="outline"
                className="mt-3 text-[#FF6300] hover:bg-[#FF6300] hover:text-white transition-all duration-300 hover:scale-105 hover:shadow-md"
